...
namespace Core.DAL.Services
{
	public class ReportesService : IReportes
	{
		...
		public ReportesProductoIndexViewModel GetReporteProductos(ProductoParametersViewModel parameters, int sucursalId)
		{
			...
			var detalleCompras = _context.Set<DetalleCompra>().Where(x => x.Active && x.Compra.Estado == Constants.EstadoCompra.PendientedePago && x.Compra.SucursalId == sucursalId && x.DateCreated.Date >= parameters.FechaInicio.ToUniversalTime().Date && x.DateCreated.Date <= parameters.FechaFin.ToUniversalTime().Date);
